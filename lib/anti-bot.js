...
function evaluateBlock({ text, title, url }, options = {}) {
  ...
  // 短正文 + 安全标记 → 很可能是验证码拦截页；正文充足 → 正常页，忽略词表误配
  const bodyLen = (text || '').trim().length;
  if (hit && bodyLen < 500) return { blocked: true, reason: 'marker:' + hit + ' shortLen=' + bodyLen };
  if (/\/verify|\/captcha|\/seccheck|\/sec\b/i.test(url || '')) return { blocked: true, reason: 'url' };
  if (bodyLen < minLen) return { blocked: true, reason: 'short-body:' + bodyLen };
